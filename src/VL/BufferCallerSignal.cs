...
namespace VVVV.Audio.Signals
{
    ...
    public class MultiBufferCallerSignal : MultiChannelSignal
    {
        internal AudioBufferStereo StereoBuffer = new AudioBufferStereo();
        Time FTime;
        List<AudioSignal> Input;
        ...
        public MultiBufferCallerSignal()
        {
            Input = new List<AudioSignal>();
            Input.Add(new SilenceSignal());
            Input.Add(new SilenceSignal());
    ...
    public class MultiBufferGeneratorSignal : MultiChannelSignal
    {
        internal AudioBufferMulti Buffer = new AudioBufferMulti();
        Time FTime;
        //List<AudioSignal> Input;
        //AudioSignal[] SilenceSignals = new AudioSignal[2];

        public MultiBufferGeneratorSignal(int channelCount = 2)
        {
            //Input = new List<AudioSignal>();
            //Input.Add(new SilenceSignal());
            //Input.Add(new SilenceSignal());
            //Input.CopyTo(SilenceSignals);
            SetOutputCount(channelCount);
            Buffer.Channels = channelCount;
        }
        ...
        public Action<AudioBufferMulti> PerBuffer;
        ...
        protected override void FillBuffers(float[][] buffer, int offset, int count)
        {
            //lock (FLock)
            {
                ...
                if (perBuffer != null)
                {
                    ...
                    //leftIn.Read(buffer[0], offset, count);
                    //rightIn.Read(buffer[1], offset, count);
                    Buffer.PrepareBuffer(buffer, FTime);
                    perBuffer(Buffer);
                }
                else
                {
